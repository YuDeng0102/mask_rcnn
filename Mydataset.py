...
class TreeDetection(data.Dataset):
    ...
    def __init__(self, root, dataset="train", transforms=None):
        super(TreeDetection, self).__init__()
        ...
        self.anno_path= self.anno_path.replace("\\","/")
        ...
        if dataset == "train":
            ...
            with open("Tree_indices.json", "w") as f:
                f.write(json_str)
...
if __name__ == '__main__':
    ...
    train = TreeDetection("data/WenCounty_Mycoco3", dataset="train")
    print(len(train))
    t = train[0]